...
filter :books_in_select_content_bucket, #_in_project_select, :as => :select, 
  ...
  action_item :only => :index do
    ...
    index do
        ...
        column("App Status") {|book| status_tag(book.appstatus) }
        column("ASIN") do |book|
    ...
    csv do
        ...
        column("Status on ereader") { |book| book.status }
        column("Status on app") { |book| book.appstatus }
    ...
    form do |f|
    	f.inputs "Book Details" do
            ...
            f.input :status, :collection => Book.books_status_collection 
